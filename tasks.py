...
def create_report_task(reporter, collect_task, analyze_task, fact_check_task):
    return Task(
        description=(
            ...
            "### 2. 核心主题\n"
            "**这是报告主体，每个主题写 8-12 句，叙事要具体、有细节。**\n"
            "每个主题按以下结构展开:\n"
            "- 发生了什么（具体事件、参与方、时间、地点）\n"
            "- 背景是什么（为什么发生、行业/政策背景）\n"
            "- 意味着什么（影响分析、趋势信号、关联事件）\n"
            "- 有哪些值得关注的数据或细节\n"
            "提取 2-3 个最重要的主题，覆盖多篇文章的交叉信息。\n"
            "每个主题不少于 200 字。避免空洞的概括，填充具体事实。\n\n"
            "### 3. 风险与挑战\n"
        ...
        expected_output=(
            "A Chinese markdown report with exactly 5 sections: 执行摘要, "
            "核心主题(每主题≥200字具体叙事), 风险与挑战, 关键结论, "
            "参考来源(with real URLs). AI disclaimer at bottom."
        ),
        agent=reporter,
